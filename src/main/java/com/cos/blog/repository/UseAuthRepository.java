...
public interface UseAuthRepository extends JpaRepository<UseAuth, Integer>{
	//SELECT * FROM userWHERE username = ?
	/*
	 * Optional<User> findByUsername(String username); Page<User>
	 * findByUsernameContaining(String keyword, Pageable pageable); Page<User>
	 * findByNickNameContaining(String keyword, Pageable pageable);
	 */
	UseAuth findByUserAndAuthDelYn(User user, String authDelYn);
